...
## Next steps / extras I can add
- Add a short health-check script which waits for MinIO to be available before starting other services.
- Add a tiny `minio-init.sh` host script that runs `mc` commands to verify the bucket and policies.

---

If you'd like, I can create a small `minio-check.sh` script and add it to the repo and the README — tell me if you want that and I'll add it next.